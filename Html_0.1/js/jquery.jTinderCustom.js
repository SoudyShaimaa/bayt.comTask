...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		dislike: function() {
			panes.eq(current_pane).find($that.settings.dislikeSelector).css('opacity', 0.9);
		...
		like: function() {
			panes.eq(current_pane).find($that.settings.likeSelector).css('opacity', 0.9);
			...
			switch (ev.type) {
				...
				case 'touchmove':
					if(touchStart === true) {
						...
						var opa = (Math.abs(deltaX) / $that.settings.threshold) / 100 - 0.0;
						if(opa > 1.0) {
							opa = 1.0;
						}
						if (posX >= 0) {
						...
						if(posX > 0 || posX < 0){
							$(".more-button").css("z-index", "0");
						}else if(posX = 0){
							$(".more-button").css("z-index", "2")
						}
					}
					break;
				case 'mouseup':
				case 'touchend':
					...
					posX = deltaX + lastPosX;
					posY = deltaY + lastPosY;
					var opa = Math.abs((Math.abs(deltaX) / $that.settings.threshold) / 100 + 0.0);
